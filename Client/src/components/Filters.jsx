...
const Filters = () => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        const res = await fetch(
          `http://localhost:3000/api/query/users?${searchQuery}`
        );
  ...
  return (
    <div className="flex flex-col md:flex-row gap-3 p-5 pl-0">
      ...
      <div>
        {createTeam && (
          <button className="bg-slate-700 text-white p-3 rounded-lg hover:opacity-50 active:opacity-70 fixed bottom-10 right-10" onClick={handleTeamSelection}>
            Proceed
          </button>
        )}
      </div>
